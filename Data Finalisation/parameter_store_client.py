...
def get_aws_credentials(conn_id):
    ...
    # For Airflow, you can also use BaseHook.get_connection(conn_id) if needed
    ...
    return login, password, region

class EC2ParameterStore:
    def __init__(self, conn_id="parameter_store_client_aws", **client_kwargs):
        aws_access_key_id, aws_secret_access_key, region = get_aws_credentials(conn_id)
        
        self.client = boto3.client('ssm', aws_access_key_id=aws_access_key_id,
                           aws_secret_access_key=aws_secret_access_key,
                           region_name=region, **client_kwargs)
        self.path_delimiter = '/'
