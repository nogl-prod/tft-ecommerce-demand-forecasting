...
model_no_rolling_path = "/opt/ml/processing/input2/trained_models/tft/no-rolling/stoertebekker/"
uploadsql = pd.DataFrame([["read model no rolling path",model_no_rolling_path]], columns=["step","path"])
uploadsql.to_sql("model_loading1", con = engine, schema="forecast_test", if_exists="replace", index=False, chunksize=1000, method="multi")

model_rolling_path = "/opt/ml/processing/input2/trained_models/tft/rolling/stoertebekker/"
uploadsql = pd.DataFrame([["read model rolling path",model_rolling_path]], columns=["step","path"])
uploadsql.to_sql("model_loading2", con = engine, schema="forecast_test", if_exists="replace", index=False, chunksize=1000, method="multi")
...
def get_results(model, data, first_prediction_time_idx, time_idx, max_encoder_length, max_prediction_length):
    ...
    time_idx_column = []
    for i in range(len(x.get("decoder_time_idx"))):
...
filename = "opt/ml/processing/input2/stoertebekker/" + today + "_TopSeller_consolidated_cutoff.csv" # uncomment after testing is completed
print(filename)
uploadsql = pd.DataFrame([["read TopSeller csv",filename]], columns=["step","path"])
uploadsql.to_sql("read_data1", con = engine, schema="forecast_test", if_exists="replace", index=False, chunksize=1000, method="multi")
...
# load data from .csv file
filename = "opt/ml/processing/input2/stoertebekker/" + today + "_LongTail_consolidated_cutoff.csv" # uncomment after testing is completed
print(filename)
uploadsql = pd.DataFrame([["read LongTail csv",filename]], columns=["step","path"])
uploadsql.to_sql("read_data2", con = engine, schema="forecast_test", if_exists="replace", index=False, chunksize=1000, method="multi")
...
# load data from .csv file
filename = "opt/ml/processing/input2/stoertebekker/" + today + "_Kicked_consolidated_cutoff.csv" # uncomment after testing is completed
print(filename)
uploadsql = pd.DataFrame([["read LongTail csv",filename]], columns=["step","path"])
uploadsql.to_sql("read_data2", con = engine, schema="forecast_test", if_exists="replace", index=False, chunksize=1000, method="multi")
...
last_observation_day = "2023-01-24"
...
t = Timer("Upload")
forecasts_consolidated.to_sql("forecasts_28days", con = engine, schema="forecasts", if_exists='replace', index=False, chunksize=1000, method="multi")
print("uploaded forecasts_28days")
forecasts_category.to_sql("forecasts_28days_category", con = engine, schema="forecasts", if_exists='replace', index=False, chunksize=1000, method="multi")
print("uploaded forecasts_28days_category")
forecasts_incl_history.to_sql("forecasts_28days_incl_history", con = engine, schema="forecasts", if_exists='replace', index=False, chunksize=1000, method="multi")
