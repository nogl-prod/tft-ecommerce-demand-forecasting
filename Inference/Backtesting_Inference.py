...
parser.add_argument("--last_observation", type=str, required=True)
parser.add_argument("--encoder_len", type=str, required=True)
parser.add_argument("--prediction_len", type=str, required=True)
parser.add_argument("--model_rolling", type=str, required=True)
parser.add_argument("--model_no_rolling", type=str, required=True)
parser.add_argument("--top_seller", type=str, required=True)
parser.add_argument("--long_tail", type=str, required=True)
parser.add_argument("--kicked", type=str, required=True)
parser.add_argument("--table_name", type=str, required=True)
# Parse the argument
args = parser.parse_args()
# get config params
section = args.client_name
encoder_len = int(args.encoder_len)
prediction_len = int(args.prediction_len)
model_rolling = args.model_rolling
model_no_rolling = args.model_no_rolling
top_seller = args.top_seller
long_tail = args.long_tail
kicked = args.kicked 
table_name = args.table_name

print("client name:", section)
print('model_rolling:', model_rolling)
filename = '/opt/ml/processing/input/databaseAWSRDS.ini'
params = config(section="stoertebekker-test",filename=filename)

# no_rolling refers to top seller model and rolling refers to long tail model
model_no_rolling_path = "/opt/ml/processing/input2/backtesting/checkpoints/" + model_no_rolling
model_rolling_path = "/opt/ml/processing/input2/backtesting/checkpoints/" + model_rolling

best_no_rolling = model_no_rolling_path #get_best_model(model_no_rolling_path)
best_rolling = model_rolling_path #get_best_model(model_rolling_path)
...
def prepare_data_for_inference(data, last_observation_day, max_prediction_length = 90, max_encoder_length = 380): # outsource
...
def get_results(model, data, first_prediction_time_idx, time_idx, max_encoder_length, max_prediction_length):
    ...
    print("Length of time_idx_column: ", len(time_idx_column))
...
def _read_data_from_s3( file_path: str) -> pd.DataFrame:
    """
    ...
    bucket_name, key = _parse_s3_path(file_path)
...
data_top_seller = _read_data_from_s3(top_seller)
...
data_long_tail = _read_data_from_s3(long_tail)
...
data_extreme_long_tail = _read_data_from_s3(kicked)
...
last_observation_day =  args.last_observation #str(yesterday) #"2022-12-28"  # should be yesterday
#last_observation_day = "2022-12-26"
...
print('data_top_seller_inf', data_top_seller_inf.shape)
# In[19]:
...
table1 = "forecasts_28days_" + table_name
table2 = "forecasts_28days_category_" + table_name
table3 = "forecasts_28days_incl_history_" + table_name

t = Timer("Upload")
forecasts_consolidated.to_sql(table1, con = engine, schema="forecasts", if_exists='replace', index=False, chunksize=1000, method="multi")
print("uploaded forecasts_28days")
forecasts_category.to_sql(table2, con = engine, schema="forecasts", if_exists='replace', index=False, chunksize=1000, method="multi")
print("uploaded forecasts_28days_category")
forecasts_incl_history.to_sql(table3, con = engine, schema="forecasts", if_exists='replace', index=False, chunksize=1000, method="multi")
print("uploaded forecasts_28days_incl_history")
t.end()
