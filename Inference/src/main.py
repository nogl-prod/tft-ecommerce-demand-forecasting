...
S3_BUCKET_PATH = ["s3://tft-training-data/halalonline/20230811_TopSeller_consolidated_cutoff.csv", "s3://tft-training-data/halalonline/20230811_LongTail_consolidated_cutoff.csv","s3://tft-training-data/halalonline/20230811_Kicked_consolidated_cutoff.csv"]
# S3_BUCKET_PATH = ["s3://fcb-bucket/dataset/20230814_TopSeller_consolidated_cutoff.csv", "s3://fcb-bucket/dataset/20230814_LongTail_consolidated_cutoff.csv","s3://fcb-bucket/dataset/20230814_Kicked_consolidated_cutoff.csv"]
LAST_OBSERVATION_DAY = "2023-08-13"
MODEL_PATH = ["s3://tft-training-data/trained_models/tft/no-rolling/halalonline/20230811_tft-epoch=02-val_loss=0.25.ckpt","s3://tft-training-data/trained_models/tft/rolling/halalonline/20230811_tft-epoch=04-val_loss=0.17.ckpt","no_model"]      # Please dont change the path as it is hard coded to check no-rolling
# MODEL_PATH = ["/opt/ml/processing/models/trained_models/tft/no-rolling/dogs-n-tiger/","/opt/ml/processing/models/trained_models/tft/rolling/dogs-n-tiger/","no_model"]      # Please dont change the path as it is hard coded to check no-rolling
# MODEL_PATH = ["/opt/ml/processing/models/models/tft/no-rolling/", "/opt/ml/processing/models/models/tft/rolling/", "no_model"]      # Please dont change the path as it is hard coded to check no-rolling
# MODEL_PATH = ["s3://fcb-bucket/models/tft/no-rolling/20230814_tft-epoch=04-val_loss=0.71.ckpt", "s3://fcb-bucket/models/tft/rolling/20230814_tft-epoch=04-val_loss=0.10.ckpt", "no_model"]      # Please dont change the path as it is hard coded to check no-rolling

CLIENT_NAME = "halalonline"
    ...
    try:
        if bundle == "True":
            ...
            with log_time("Importing plan data"):
                downloader = DatabaseManager(client_name = client_name)
                bundle_matrix = downloader.import_data_from_aws_rds(schema=SCHEMA_FORECAST, table_name=BUNDLE_MATRIX)
                logging.info("Bundle matrix imported")
                # Remove the 'bundle_type' column
                bundle_matrix = bundle_matrix.drop('bundle_type', axis=1)
            # Data uploading
            ...
            with log_time("Preparing Forecast bundle results"):
                ...
                # Change categorical columns to string type
                categorical_columns = results.select_dtypes(include=['category']).columns
                results[categorical_columns] = results[categorical_columns].astype(str)
        ...
        with log_time("Data uploading"):
            ...
            uploader.upload_data(results, schema=SCHEMA_FORECAST, table_name=table_name)
            if bundle == "True":
                uploader.upload_data(historic_bundle_results, schema=SCHEMA_HISTORIC, table_name="historic_bundle_results")
                uploader.upload_data(historic_bundle_results, schema=SCHEMA_SC, table_name="historic_bundle_results")
