...
class ProductAnalysis:
    ...
    def __init__(self, param_store_path, days=None, reduction="median", data_path=None, data=None):
        """
        Parameters
        ----------
        data_path : str
...
#             self.data = self.data.groupby(['created_at', 'SC_variant_id']).sum().reset_index()
    ...
    def compute_zero_plateau(self):
        ...
        zero_plateau = self.data.groupby('SC_variant_id')['TVUR_amazon_lineitems_quantity'].apply(calculate_zero_pct)
        return zero_plateau
    ...
    def compute_cov(self):
        ...
        cov_values = self.data.groupby('SC_variant_id')['TVUR_amazon_lineitems_quantity'].apply(calculate_cov)
        return cov_values
    ...
    def compute_cot(self):
        ...
        cot = self.data.groupby('SC_variant_id')['TVUR_amazon_lineitems_quantity'].apply(calculate_cot)
        return cot
    ...
    def compute_seasonality(self, period):
        ...
        seasonality = self.data.groupby('SC_variant_id')['TVUR_amazon_lineitems_quantity'].apply(extract_seasonality)
        return seasonality
    ...
    def rank_products(self):
        ...
        def calculate_revenue(series):
            trimmed_series = ProductAnalysis.trim_series_from_first_non_zero(series)
            return trimmed_series['TVKR_variant_RRP'] * trimmed_series['TVUR_amazon_lineitems_quantity']

        self.data['revenue'] = self.data.groupby('SC_variant_id').apply(calculate_revenue)

        sales_rank = self.data.groupby('SC_variant_id')['TVUR_amazon_lineitems_quantity'].sum().rank(ascending=False)
    ...
    def flag_importance(self):
        ...
        self.data['revenue'] = self.data['TVKR_variant_RRP'] * self.data['TVUR_amazon_lineitems_quantity']

        # Rank products by number of sales and revenue volume
        sales_rank = self.data.groupby('SC_variant_id')['TVUR_amazon_lineitems_quantity'].sum().rank(pct=True)
