...
# Warning: Change this variable to your data source
...
# ## 2.3 Transform & clean data

# In[8]:


# Image("../../Data Samples/Google Analytics/GA grouping levels.png")

# level 1 would be the GA_main vs. GA_pdp. main need to be merged on every pdp row later. So same for every product / variant
# level 2 would be information from ga_channelgrouping
# level 3 would be information from ga_source or ga_campaign if it is not split in level 2 and level 2 rest, but has an acutal detailled breakdown
# level 4 is necessary for splitting social level 2 in paid and organic via ga_campaign == (not set) vs. ga_campaign != (not set)


# ### 2.3.1 Level 2
# 
# Use ga_channelgrouping as the 2nd grouping level
# 
# - Paid Search: always refers to ga_source == "google". Extract level 3 split in Search, Smart Shopping etc. with ga_campaign -> e.g. run: GA_main.loc[GA_main["ga_channelgrouping"] == "Paid Search"].ga_campaign.unique()#
# - (Other): incl. several "rest"-like sources via ga_source, but also ig', 'fb', 'email', 'facebook', 'google'. The main ones need to be extracted and then the rest should be consolidated under one level 2 feature "rest".
# - Referral: create one level 2 feature from this aggregating all. No need for level 3 split
# - Social: use breakdown as seen in picture above with ga_source and put everything else in Social-Rest feature level 2 and 3 combination
# - Organic Search: split in feature level 3 google and rest with ga_source
# - Direct: no feature split in level 3 necessary --> Leave as one aggregated level 2 feature
# - Email: no feature split in level 3 necessary --> Leave as one aggregated level 2 feature
# - Display: always refers to Google (Youtube). no feature split in level 3 necessary

# <br>
# 
# 
# #### Decisions about ga_source of ga_channelgrouping == (Other)
# 
# "ig","fb","facebook","Facebook_Ads" >> "Social"
# 
# "google" >> "Paid Search"
# 
# "email" >> "Email"
...
# 
# 
# Some times ga_source is a number when ga_channelgrouping is "(Other)". This number has the same initials than the instagram campaign id, and it was considered as instagram
...
# ## 2.4 ga_main ga_pdp Transformation
# 
# ### ga_main decisions made:
# 
# Features agregated with sum: ['ga_users', 'ga_newusers', 'ga_sessions','ga_pageviews', 'ga_uniquepageviews'] 
# 
# Features agregated with mean: ['bouncerate','avgtimeonpage','sessionsperuser','avgsessionduration','pageviewspersession']
#     
# ### ga_pdp decisions made:
# 
# Features agregated with sum: ['itemrevenue','productcheckouts','productdetailviews','quantitycheckedout','productaddstocart','quantityaddedtocart','productremovesfromcart']
# 
# Features agregated with mean: ['buytodetailrate','carttodetailrate','itemsperpurchase']
# 
#     
...
# Please validate the list of columns to apply forward fill and backward fill as **ffill_cols** and **bfill_cols**.
...
df_final = merge_with_sceleton(df1,df2)
t.end()
...
df_final.head(5).to_sql('google_analytics', con = engine, schema="transformed", if_exists='replace',index=False) #drops old table and creates new empty table


# In[20]:


engine.execute('TRUNCATE TABLE transformed.google_analytics') #Truncate the table in case you've already run the script before
...
df_final.to_csv('/opt/ml/processing/input/Data Source Transformations/Data Uploads/google_analytics.csv', index=False, header=False) #Name the .csv file reference in line 29 here
...
with open('/opt/ml/processing/input/Data Source Transformations/Data Uploads/google_analytics.csv', 'r') as f:    
    ...
    cursor = conn.cursor()
    cmd = 'COPY transformed.google_analytics('+ col_names+ ')'+ 'FROM STDIN WITH (FORMAT CSV, HEADER FALSE)'
